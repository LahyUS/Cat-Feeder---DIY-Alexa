...
{
    if (intent.intent_confidence < 0.80)
    {
    ...
    if (intent.device_confidence < 0.80)
    {
    ...
    if (intent.trait_confidence < 0.7)
    {
    ...
    else
    {
        ...
        Serial.printf("Device %s (%i) is %d --- with score: %f\n", intent.device_name.c_str(), m_device_to_pin.find(intent.device_name), is_turn_on, intent.device_confidence);
    }
    // success!
    return SUCCESS;
}
