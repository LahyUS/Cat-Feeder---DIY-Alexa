...
void setup()
{
  // Setup LED_BUILT_IN for demo
  gpio_pad_select_gpio(LED_OUT);
  ...
  intent_processor->addDevice("food", GPIO_NUM_5);
  intent_processor->addDevice("stupidcat", GPIO_NUM_21);
  intent_processor->addDevice("foodie", GPIO_NUM_23);
...
void loop()
{
  String recieve_flag = Serial1.readString();
  int device_activ_flag = (int)recieve_flag[0];
  if(device_activ_flag == 0);
  {
    digitalWrite(23, LOW);
  }
